...
void processCamera(bool showWindow, unsigned int method,
                    unsigned int unnormal = 10, unsigned int fps = 24 ){
    //CvCapture *capture = cvCaptureFromCAM(0);          //get camera device    
...
if(!capture){
    ...
    bool backGroundFlag = true, recordFlag = false;                    //judge the first frame
    Mat frame, fgMask;                                     //current frame
    Mat HSV;                                      //HSV format
    ...
    while ((char)keyboard != 'q' && (char)keyboard != 27){
        ...
        frame = cvQueryFrame(capture);             //get frame from device
        if (method == 0)
        ...
        //set background
        if(backGroundFlag){
            cvtColor(frame, HSV, CV_BGR2HSV);        //frame tranform to HSV
            calcHist(&HSV, 1, channels, Mat(), base, 2, histSize, range, true, false); //cout histogram for HSV 
            normalize(base, base, 0, 1, NORM_MINMAX, -1, Mat());   // Min-Max Normalization
            backGroundFlag = false;
        }
       //
        cvtColor(frame, HSV, CV_BGR2HSV);
        calcHist(&HSV, 1, channels, Mat(), cur, 2, histSize, range, true, false);
        ...
        // Here We Starting Recording
        if(recordFlag){
            cvWriteFrame(recorder, &(IplImage(frame)));
        }
        
        if (showWindow && !fgMask.empty()){
            imshow("Monitor",fgMask);              //output to window of monitor,monitor will be created by main
        }
        keyboard = waitKey(30);                   //wait 30ms if anykey input return the ASCII of the key
        
    }
    cvReleaseVideoWriter(&recorder);
    cvReleaseCapture(&capture);                 // release camera
}
