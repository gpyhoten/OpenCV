...
int main(int argc, char* argv[]){
    ...
    if (argc > 6){
        cerr << "Invalid Parameter, Exiting..." << endl;
        exit(EXIT_FAILURE);
    }
    if (argc >= 2){
    ...
    pMOG2 = new BackgroundSubtractorMOG();           //MOG2 approach
...
void help(){
	cout << "-----------------------------------------------------------------------------\n"
		...
		<<"          -mog1 show the MOG frame;                                     \n"
		<<"          -mog2 show the MOG2 frame;                                    \n"
		<<" [FPS] : set the fps of record file, default is 24.                     \n"
		<<" [THRESHOLD] : set the number x that the monitor will start recording after \n"
		<<"               x unnormal frames passed.                                 \n"
		<<" [OUTPUTFILE] : assign the output recording file. It must be .avi format. \n "
		<<"                           designed by Forec                             \n "
		<<"---------------------------------------------------------------------------\n";
